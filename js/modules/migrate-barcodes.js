...
(async function () {
    ...
    products.forEach(function (p) {
        if (p.code && /^PY-\d+$/i.test(String(p.code))) {
    ...
    const confirmed = confirm(
        "برای " + products.length + " کالا فیلد barcode اضافه/به‌روز شود؟\n" +
    ...
    await new Promise(function (resolve, reject) {
        ...
        products.forEach(function (product) {
            let barcode = String(product.code || "").trim();

            if (!barcode) {
                barcode = "PY-" + String(nextNum).padStart(3, "0");
        ...
        tx.onerror = function () { reject(new Error("ذخیره بارکدها انجام نشد.")); };
    });
